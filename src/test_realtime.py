...
model_resnet = DETR(num_classes=22)
model_resnet.eval()
model_resnet.load_pretrained('E:\\git\\SignDETR\\pretrained\\best_model_resnet.pt')
...
while cap.isOpened(): 
    ...
    result = model_resnet(torch.unsqueeze(transformed['image'], dim=0))
    inference_time = (time.time() - inference_start) * 1000  # Convert to ms

    probabilities = result['pred_logits'].softmax(-1)[:,:,:-1] 
    max_probs, max_classes = probabilities.max(-1)
    keep_mask = max_probs > 0.2
